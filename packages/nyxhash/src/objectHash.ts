...
function createHasher(options: HashOptions) {
   ...
   return {
      ...
      object(object) {
         ...
         const objectNumber = context.get(object)
        ...
        && objType !== 'asyncfunction'
            ...
            let extraKeys: readonly string[] = defaultPrototypesKeys
            ...
            if (options.excludeKeys) {
               const excludeFn = options.excludeKeys ?? (() => false)
               keys = keys.filter(key => !excludeFn(key))
               extraKeys = extraKeys.filter(key => !excludeFn(key))
            }
